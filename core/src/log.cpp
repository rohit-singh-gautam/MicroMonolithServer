...
std::atomic<size_t> logger::logger_nextid { 0 };

void logger_list::add(logger *new_logger)
{
    if (enabled) {
        std::lock_guard guard {mutex};
        logger_store.insert({new_logger->GetID(), new_logger});
    }
}
...
void logger_list::remove(logger *new_logger)
{
    if (enabled) {
        std::lock_guard guard {mutex};
        logger_store.erase(new_logger->GetID());
    }
}
...
void logger_list::flush() {
    if (enabled) {
        std::lock_guard guard {mutex};
        std::ranges::for_each(logger_store, [this](auto &logger) { logger.second->flush(this->fd); });
    }
    sync();
}
...
template <typename T, T radix = 10, number_case_t number_case = number_case_t::lower>
...
void floatToStringHelper(char *&pStr, const uint8_t *&data_args) {
    auto value = *(FloatType *)data_args;
    data_args += sizeof(FloatType);
...
void ipv6_port_t_to_string_helper(char *&pStr, const uint8_t *&data_args) {
    ...
    uint16_t port = value;
    auto count =  to_string<uint16_t, 10, number_case_t::lower, false>(port, pStr);
...
void errno_to_string(char *&pStr, const uint8_t *&data_args) {
    ...
    *pStr++ = '(';
    auto count =  to_string<int32_t, 10, number_case_t::lower, false>(errnum, pStr);
    pStr += count;
    *pStr++ = ',';
...
void logger_level_to_string_helper(char *&pStr, const uint8_t *&data_args) {
    const logger_level_t &value = *reinterpret_cast<const logger_level_t *>(data_args);
    data_args += sizeof(logger_level_t);
    auto count =  to_string<false>(value, pStr);
...
void epoll_event_to_string_helper(char *&pStr, const uint8_t *&data_args) {
    ...
    *pStr++ = '(';
    auto count =  to_string<int32_t, 10, number_case_t::lower, false>(event, pStr);
    pStr += count;
    *pStr++ = ',';
    ...
    *pStr++ = ':';
    ...
    while(*desc_str) {
        ...
        case formatstring_state::SPECIFIER:
            ...
            case 'X': {
                switch(lenght_specifier) {
                case formatstring_modifier::NONE: integerToStringHelper<uint32_t, 16, number_case_t::upper>(pStr, data_args); break;
                case formatstring_modifier::h: integerToStringHelper<uint16_t, 16, number_case_t::upper>(pStr, data_args); break;
                case formatstring_modifier::hh: integerToStringHelper<uint8_t, 16, number_case_t::upper>(pStr, data_args); break;
                case formatstring_modifier::l:
                case formatstring_modifier::ll: integerToStringHelper<uint64_t, 16, number_case_t::upper>(pStr, data_args); break;
                default:;
                };
                state = formatstring_state::COPY;
                break;
        ...
        case formatstring_state::SPECIFIER_CUSTOM:
            switch (c) {
                ...
                case 'N': ipv6_socket_addr_t_to_string_helper<number_case_t::upper>(pStr, data_args); break;
                case 'i': ipv6_addr_t_to_string_helper(pStr, data_args); break;
                case 'I': ipv6_addr_t_to_string_helper<number_case_t::upper>(pStr, data_args); break;
...
void active_module::set_module(const module_t module, const logger_level_t level) {
...
void active_module::enable_all(const logger_level_t level) {
